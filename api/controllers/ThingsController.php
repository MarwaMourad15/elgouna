...
class ThingsController extends ApiController {
	...
	public function actionCategory() {
		$cats = array ();
		$categories = BeachCategory::find ()->orderBy ( [ 
				'ordering' => SORT_ASC 
		] )->all ();
	...
	public function actionListThings() {
		...
		if (isset ( $params ['visaCheck'] ) && ($params ['visaCheck'] != '')) {
			...
			$query->andWhere ( [ 
					'isVisaPaymentAvailable' => $visaCheck 
			] );
		}
		...
		if (isset ( $params ['wifiCheck'] ) && ($params ['wifiCheck'] != '')) {
			...
			$query->andWhere ( [ 
					'isWifiAvailable' => $wifiCheck 
			] );
		}
		
		$all = array ();
		$beaches = $query->limit ( $limit )->offset ( $start )->orderBy ( [ 
				'ord' => SORT_ASC 
		] )->all ();
		...
		foreach ( $beaches as $row ) {
			...
			if (isset ( $params ['userId'] ) && $params ['userId'] != "") {
				$n_like = UserBeachLike::find ()->where ( [ 
	...
	public function actionGetThingReviews() {
		...
		if (isset ( $params ['beachId'] ) && $params ['beachId'] != '') {
			...
			$query = BeachReview::find ()->where ( [ 
					'approved' => 1 
			] )->andWhere ( [ 
					'beach_id' => $beach_id 
			] )->limit ( $limit )->offset ( $start )->orderBy ( [ 
					'date' => SORT_ASC 
			] );
	...
	public function actionSubmitThingReview() {
		$params = $this->parseRequest ();
		if (isset ( $params ['userId'] ) && isset ( $params ['beachId'] ) && isset ( $params ['review'] ) && isset ( $params ['rating'] )) {
			$new = new BeachReview ();
			$new->user_id = $params ['userId'];
			$new->beach_id = $params ['beachId'];
			...
			$Reviews = BeachReview::findAll ( [ 
					'beach_id' => $params ['beachId'],
					'approved' => 1 
			] );
			$beach = Beaches::findOne ( [ 
					'id' => $params ['beachId'] 
			] );
			$sum = 0;
			$total = count ( $Reviews );
