...
class VenueController extends ApiController
{
    ...
    public function actionCategory()
    {
        ...
        if(isset($params['type']) ){
            $type = $params['type']; // 0 dining    1 night life
            $cats = array();
            $categories = VenueCategory::find()
                ->where(['type' => $type ])
    ...
    public function actionListVenues()
    {
        $params = $this->parseRequest();
        if(isset($params['categoryType']) ){
            $categoryType = $params['categoryType']; // 0 dining    1 night life
            ...
            if(isset($params['visaCheck']) && ($params['visaCheck']!=''))
            {
                ...
                $query->andWhere(['visaCheck'=>$visaCheck]);
            }
            ...
            if(isset($params['wifiCheck']) && ($params['wifiCheck']!=''))
            {
                ...
                $query->andWhere(['wifiCheck'=>$wifiCheck]);
            }

            $all = array();
            $venues = $query->orderBy(['ord'=>SORT_ASC])->all();
            //print_r($venues);die;
