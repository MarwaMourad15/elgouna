...
$sql44 = "select * from hotels where hidden=0 order by ratingStar desc";
//echo $sql44;
...
$end = $start + $limit;
$sql = "select * from hotels where hidden=0 order by ratingStar desc limit $start,$limit";
                ...
                while ($row = mysql_fetch_array($result)) {
                    ...
                    <tr <?php if ($e == 0) { ?> style="background-color:#EFEFEF" <?php } ?>>
                        ...
                        <td width="400" style=" width:400px;">
                            ...
                            if($n_re > 0) {
                            ?>
                            <a href="hotelReviews.php?id=<?php echo $row['id']; ?>">
                            <?php
                            echo $n_re;
                                if($n_re > 1) {
                                ?> 
                        ...
                        <td width="26">
                            <a href="javascript:addNewItem('createNewHotel.php?id=<?php echo $row['id']; ?>','<?php echo $_POST['pgid']; ?>');"><i class="fa fa-pencil"></i></a>
                            <a href="#myModal" onclick="deleteItem('deleteHotel.php', '<?php echo $row['id']; ?>', '<?php echo $_POST['pgid']; ?>', '1')" role="button" data-toggle="modal"><i class="fa fa-remove"></i></a> <?php if ($row['hidden'] == 1) { ?><i class="icon-remove-circle"></i><?php } ?>         </td>
                    </tr>
                <?php }
                ?>
            </tbody>
